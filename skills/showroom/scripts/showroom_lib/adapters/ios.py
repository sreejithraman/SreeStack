"""showroom integration for the safe iOS Simulator adapter."""
...
class Adapter:
    """showroom loader integration around the independently testable adapter."""
    ...
    def recover(
        ...
        matches = [
            ...
            and item.showroom_id == request.showroom_id
            and item.project_identity == request.project_identity
            and item.worktree_identity == request.worktree_identity
        ]
        if len(matches) > 1:
            raise CoreAdapterError("multiple manager-owned Simulator clones match interrupted showroom start")
        if not matches:
            return None
        try:
    ...
    def _configuration(context: AdapterContext | None) -> IOSConfiguration:
        raw = dict(context.config.ios) if context is not None else {}
        ...
        if not isinstance(launch_arguments, (list, tuple)) or not all(
            ...
            raise CoreAdapterError("config ios.launch_arguments must be a string array")
    ...
    @staticmethod
    def _stored_result(record: dict[str, Any]) -> IOSShowroomResult:
