...
class IOSDiscoveryMixin:
    def discover_container(self, request: IOSShowroomRequest) -> XcodeContainer:
    ...
    def discover_shared_scheme(
        self, request: IOSShowroomRequest, container: XcodeContainer
    ) -> str:
    ...
    def _attempt_directory(self, request: IOSShowroomRequest) -> Path:
        timestamp = self._clock().astimezone(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
        safe_showroom_id = re.sub(r"[^A-Za-z0-9_.-]+", "-", request.showroom_id)
        return (
