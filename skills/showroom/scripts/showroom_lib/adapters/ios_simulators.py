...
class IOSSimulatorLifecycleMixin:
    def _allocate_or_reuse_clone(
        self, request: IOSShowroomRequest, spec: SimulatorSpec
    ) -> SimulatorOwnership:
        ...
        matching_clones = [
            ...
            and record.showroom_id == request.showroom_id
        ...
        if templates:
            ...
            template_name = self._safe_name(
                f"showroom template {spec.device_name} {spec.runtime_name}"
            )
        ...
        clone_name = self._safe_name(
            f"showroom {request.showroom_id} {request.worktree_identity[:8]}"
        )
        clone = self._checked(
    ...
    def _assert_cleanup_authority(
        request: IOSShowroomRequest, record: SimulatorOwnership
    ) -> None:
        ...
        if record.showroom_id != request.showroom_id:
            raise OwnershipDrift("cleanup showroom identity does not match ownership")
