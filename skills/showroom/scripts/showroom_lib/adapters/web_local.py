...
class Adapter:
    ...
    def start(self, record: dict[str, Any], context: AdapterContext) -> dict[str, Any]:
        ...
        if not context.config.command:
            raise ConfigurationError("web-local showroom requires a development command")
        ...
        plist_path = context.showroom_dir / "origin.plist"
        stdout_path = context.showroom_dir / "origin.stdout.log"
        stderr_path = context.showroom_dir / "origin.stderr.log"
        command = tuple(
            item.replace("{port}", str(origin_port)).replace("{host}", LOOPBACK)
            for item in context.config.command
        )
        ...
        ownership_path = context.showroom_dir / "ownership.json"
        off_argv = [self.tailscale_bin, "serve", f"--https={serve_port}", "off"]
        ownership = {
            "version": 1,
            "showroom_id": record["id"],
        ...
        self.fs.mkdir(context.showroom_dir)
        ...
        try:
            ...
            local_url = self._with_health(target, context.config.health_check_path)
            if not self.http_probe.check(local_url, self.health_timeout):
                raise AdapterError(f"origin health check failed: {local_url}")

            # Allocation and mutation are serialized against showroom, but a
            ...
            remote_health = self._with_health(public_url, context.config.health_check_path)
        ...
        return {
            ...
            "resources": {
                ...
                "serve": {
                    "port": serve_port,
                    "target": target,
                    "health_path": context.config.health_check_path,
    ...
    def verify(self, record: dict[str, Any], context: AdapterContext | None) -> dict[str, Any]:
        ...
        health_path = str(serve.get("health_path") or (context.config.health_check_path if context else "/"))
    ...
    def recover(self, record: dict[str, Any], context: AdapterContext) -> dict[str, Any] | None:
        ...
        if ownership.get("version") != 1 or ownership.get("showroom_id") != record.get("id"):
            raise AdapterError("cannot recover web-local ownership metadata for another showroom")
        expected_label = self._label(str(record["id"]))
        expected_domain = f"gui/{self.uid}"
        expected_plist = context.showroom_dir / "origin.plist"
        if (
    ...
    def stop(
        ...
        *,
        ...
        return {
            ...
            "verification": {
                "status": "stale",
                "detail": "showroom stopped; prior verification no longer proves availability",
            },
        }
    ...
    def _owned_resources(
        ...
        if (
            ...
            raise AdapterError("ownership drift: launchd identity does not match the showroom ID")
        try:
            port = int(serve["port"])
        except (KeyError, TypeError, ValueError) as exc:
    ...
    def _allocate_ports(
        ...
        if isinstance(requested, int):
            ...
            origin_port = self._choose(self.origin_ports, occupied, showroom_id, "origin")
        occupied.add(origin_port)
        serve_port = self._choose(self.serve_ports, occupied, showroom_id, "Serve")
        return origin_port, serve_port

    def _choose(self, candidates: Sequence[int], occupied: set[int], showroom_id: str, kind: str) -> int:
        if not candidates:
            raise AdapterError(f"no {kind} port candidates are configured")
        offset = int(hashlib.sha256(showroom_id.encode("utf-8")).hexdigest()[:8], 16) % len(candidates)
    ...
    @staticmethod
    def _label(showroom_id: str) -> str:
        digest = hashlib.sha256(showroom_id.encode("utf-8")).hexdigest()[:20]
        return f"com.sreestack.showroom.{digest}"
    ...
    def _plist(
        ...
        payload = {
            ...
            "WorkingDirectory": str(context.config.working_directory),
