...
3. Run `showroom doctor [project] --json`. If a delivery command fails, read [delivery-command.md](references/delivery-command.md), inspect the repo's current release script, and repair the smallest checked-in command plus contract tests. Finish when doctor passes without delivery side effects.
