---
name: preview
description: Create, verify, and hand off the best available review surface after Codex changes interactive product behavior. Use for web, native mobile, desktop, API, CLI, library, infrastructure, and data projects when a reviewer needs a URL, running device, artifact, command, transcript, plan, or report with lifecycle details. Prefer private local surfaces for active work and durable hosted surfaces only when their availability is needed.
---
...
1. Detect the repository, worktree, project kind, and optional `.preview.toml`.
...
3. Select the first suitable surface:
   ...
   3. Local platform-specific preview for active or uncommitted work.
   4. Evidence-only surface when no interactive preview exists.
4. Run `previewctl doctor --json` before creating local resources. Invoke the bundled CLI as `python3 <skill-dir>/scripts/previewctl` when it is not installed on `PATH`.
...
- Choose local web preview for active web/API work unless the project already provides a usable branch preview or the user needs durable access.
...
Invoke an installed `vercel-deploy`, `netlify-deploy`, `cloudflare-deploy`, or `render-deploy` skill as a workflow. Do not reproduce its deployment commands inside this skill or `previewctl`. After deployment, normalize the provider result with `previewctl register` and record whether the provider, pull request, or `previewctl` owns cleanup.
...
- Act only on exact resources registered as preview-manager-owned. Never broadly reset Tailscale, kill by process name or port, erase ordinary simulators, or delete unrelated deployments or files.
- Run `previewctl cleanup --dry-run` before material cleanup and present the exact targets when authorization is required.
- State that local previews are unavailable while the host sleeps, is off, or loses required connectivity.

## Completion

Do not report success until `previewctl verify <id> --json` or equivalent platform verification records a result. Include limitations even when verification passes.
