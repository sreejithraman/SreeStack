...
from .paths import preview_directory
from .project import Manifest, Project, detect_project, load_manifest, preview_id
...
def choose_adapter(manifest: Manifest) -> str:
    if manifest.kind == "ios":
        return "ios-simulator"
    if manifest.kind in {"web", "api"}:
        return "web-local"
    return "evidence-only"


def lifecycle_commands(identifier: str, state: Path | None = None) -> dict[str, str]:
    executable = Path(__file__).resolve().parent.parent / "previewctl"
...
def _record_type(adapter: str, manifest: Manifest) -> str:
...
def new_record(
    project: Project, manifest: Manifest, adapter: str, current_time=None, state: Path | None = None
) -> dict[str, Any]:
    current = current_time or now()
    identifier = preview_id(project, adapter, manifest.profile)
    lease = manifest.lease_hours or LOCAL_LEASE_HOURS
    expiration = expires_at(current, lease) if manifest.cleanup_policy == "lease" else None
    return {
        "record_version": 1,
        "id": identifier,
        "type": _record_type(adapter, manifest),
        "adapter": adapter,
        "registered": False,
        "provider": None,
...
def _context(project: Project, manifest: Manifest, state: Path, identifier: str, approvals: dict[str, bool] | None = None) -> AdapterContext:
    return AdapterContext(project, manifest, state, preview_directory(state, identifier), approvals or {})
...
def _resource_scope(adapter: str, identifier: str) -> str:
    # Web previews share one Serve/listener namespace; iOS previews share one
    # Simulator-template namespace. Non-mutating/generic records need only
    # serialize operations on their own stable identity.
    return adapter if adapter in {"web-local", "ios-simulator"} else f"preview:{identifier}"
...
def resolve_id(registry: Registry, value: str | None, cwd: Path | None = None, adapter: str | None = None) -> str:
    ...
    selected = adapter or choose_adapter(manifest)
    identifier = preview_id(project, selected, manifest.profile)
    if registry.get(identifier) is None:
        raise ConfigurationError("no preview found for the current worktree; pass an ID or run previewctl start")
    return identifier


def start(registry: Registry, state: Path, cwd: Path | None = None, adapter_name: str | None = None, manifest_path: Path | None = None,
          approvals: dict[str, bool] | None = None) -> dict[str, Any]:
    project = detect_project(cwd)
    manifest = load_manifest(project, manifest_path)
    selected = adapter_name or choose_adapter(manifest)
    if manifest.cleanup_policy in {"provider", "pull-request"}:
        raise ConfigurationError(
            f"cleanup_policy={manifest.cleanup_policy!r} is only valid for a surface created and registered by its provider workflow"
        )
    identifier = preview_id(project, selected, manifest.profile)
        ...
        if existing.get("pinned") or expiration is None or parse_time(expiration) > current:
            ...
            try:
                verification_patch = adapter.verify(
                    ...
                    _context(project, manifest, state, identifier, approvals),
                )
            ...
            if not registry.update(save_reconciliation):
                raise AdapterError(f"preview changed while reconciliation was in progress; retry: {identifier}")
            if reconciled.get("verification", {}).get("status") == "passed":
                return reconciled
            if reconciled.get("lifecycle_owner") != "previewctl":
                raise AdapterError(f"preview verification did not pass and lifecycle is externally owned: {identifier}")
    ...
    if existing and existing.get("status") == "starting":
        ...
        if not stale:
            raise AdapterError(f"preview start already in progress: {identifier}")
        ...
        with registry.resource_lock(_resource_scope(selected, identifier)):
            ...
            if latest != existing:
                ...
                if recover is not None:
                    recover(copy.deepcopy(existing), _context(project, manifest, state, identifier, approvals))
                if not registry.delete_if(identifier, lambda value: value == existing):
                    raise AdapterError(f"preview changed during interrupted-start recovery: {identifier}")
        if changed:
            return start(registry, state, cwd, adapter_name, manifest_path, approvals)
        existing = None
    ...
    if existing and existing.get("status") == "stopping":
        ...
        if not stale:
            raise AdapterError(f"preview stop already in progress: {identifier}")
        stop(registry, state, identifier)
        return start(registry, state, cwd, adapter_name, manifest_path, approvals)

    record = new_record(project, manifest, selected, current, state)
    ...
    if not owned:
        if reserved.get("status") in {"starting", "stopping"}:
            raise AdapterError(f"preview lifecycle operation already in progress: {identifier}")
        return reserved
    context = _context(project, manifest, state, identifier, approvals)
    preview_dir_existed = context.preview_dir.exists()
    context.preview_dir.mkdir(parents=True, mode=0o700, exist_ok=True)
    if not preview_dir_existed:
        context.preview_dir.chmod(0o700)
    try:
        with registry.resource_lock(_resource_scope(selected, identifier)):
            ...
            if not registry.update(commit):
                ...
                try:
                    ...
                    pass
                raise AdapterError(f"preview start was superseded by another attempt: {identifier}")
        return completed
    except Exception as exc:
...
def get_record(registry: Registry, identifier: str) -> dict[str, Any]:
    ...
    if record is None:
        raise ConfigurationError(f"preview not found: {identifier}")
    return record
...
def _record_context(record: dict[str, Any], state: Path) -> AdapterContext | None:
    ...
    try:
        ...
        manifest_path = Path(record["manifest"]) if record.get("manifest") else None
        manifest = load_manifest(project, manifest_path)
    except (ConfigurationError, OSError):
        return None
    return _context(project, manifest, state, record["id"])
...
def verify(registry: Registry, state: Path, identifier: str) -> dict[str, Any]:
    ...
    if record.get("status") in {"stopping", "stopped"}:
        raise AdapterError(f"preview is not active: {identifier}")
    ...
    def commit(data: dict[str, Any]) -> dict[str, Any]:
        ...
        if any(current.get(field) != record.get(field) for field in lifecycle_fields):
            raise AdapterError(f"preview lifecycle changed during verification; retry: {identifier}")
...
def renew(registry: Registry, identifier: str, lease_hours: float | None = None) -> dict[str, Any]:
    ...
    def operation(data: dict[str, Any]) -> dict[str, Any]:
        ...
        if record.get("status") in {"starting", "stopping"}:
            raise AdapterError(f"preview lifecycle operation is in progress: {identifier}")
...
def _stop_locked(registry: Registry, state: Path, identifier: str, remove: bool = False,
    ...
    if record.get("status") == "stopped":
        ...
        return record
    if not record.get("registered") and record.get("lifecycle_owner") != "previewctl":
        raise AdapterError(f"preview lifecycle is owned by {record.get('lifecycle_owner')!r}")
    ...
    if not registry.update(commit):
        raise AdapterError(f"preview record changed while stop was in progress: {identifier}")
    if remove:
...
def cleanup(registry: Registry, state: Path, dry_run: bool) -> dict[str, Any]:
    ...
    for item, action in zip(candidates, actions):
        ...
        try:
            ...
            # lifecycle expires, but previewctl never claims to delete them.
...
def register(registry: Registry, project: Project, *, adapter: str, profile: str, record_type: str, provider: str | None,
    ...
    current = now()
    identifier = preview_id(project, adapter, profile)
    ...
    def operation(data: dict[str, Any]) -> dict[str, Any]:
        ...
        if existing:
            ...
            if existing.get("status") in {"starting", "stopping"}:
                raise AdapterError(f"registered preview lifecycle operation is in progress: {identifier}")
            prior_identity = (
