...
from ..project import Manifest, Project
...
class EvidenceAdapter:
    ...
    def start(self, record: dict[str, Any], context: AdapterContext) -> dict[str, Any]:
        command = list(context.manifest.command) if context.manifest.command else None
        return {
            "status": "active",
...
class RegisteredSurfaceAdapter:
    ...
    def start(self, record: dict[str, Any], context: AdapterContext) -> dict[str, Any]:
        raise RuntimeError("registered surfaces must be created before previewctl registration")
    ...
    def verify(self, record: dict[str, Any], context: AdapterContext | None) -> dict[str, Any]:
        ...
        if url:
            ...
            detail = "device evidence exists" if ok else "registered device was not exercised by previewctl"
            status = "passed" if ok else "blocked"
        elif command:
            evidence = [Path(value) for value in record.get("evidence_paths", [])]
            ok = any(path.is_file() and path.stat().st_size > 0 for path in evidence)
