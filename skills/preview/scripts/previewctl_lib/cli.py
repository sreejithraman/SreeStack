...
from .errors import PreviewctlError
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="previewctl", description="Manage provider-neutral preview handoffs")
    parser.add_argument("--version", action="version", version=f"%(prog)s {__version__}")
    parser.add_argument("--json", action="store_true", help="emit machine-readable JSON")
    parser.add_argument("--state-dir", type=Path, help=argparse.SUPPRESS)
    commands = parser.add_subparsers(dest="command", required=True)

    start_parser = commands.add_parser("start", help="start or return the current worktree preview")
    ...
    register_parser.add_argument("--lifecycle-owner", choices=("previewctl", "provider", "pull-request", "pr"), default="provider")
    ...
    try:
        if args.command == "start":
            ...
            if args.lease_hours is not None and args.lease_hours <= 0:
                raise PreviewctlError("lease hours must be positive")
