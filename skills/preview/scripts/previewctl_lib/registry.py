...
def _validate_record(record: dict[str, Any], path: Path) -> None:
    ...
    if record["lifecycle_owner"] not in {"previewctl", "provider", "pull-request", "manual"}:
...
class Registry:
    ...
    def get(self, preview_id: str) -> dict[str, Any] | None:
        record = self.read()["records"].get(preview_id)
    ...
    def delete_if(self, preview_id: str, predicate: Callable[[dict[str, Any]], bool]) -> bool:
        def operation(data: dict[str, Any]) -> bool:
            current = data["records"].get(preview_id)
            if current is None or not predicate(current):
                return False
            del data["records"][preview_id]
            return True
