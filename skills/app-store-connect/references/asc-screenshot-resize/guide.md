...
Use this guide to prepare screenshots for App Store Connect. Do not rely on a hard-coded dimension table in this guide; the CLI owns the current size matrix.
...
macOS screenshots can contain hidden Unicode spaces that make tools fail with "not a valid file". Copy the selected screenshots into a working directory and sanitize those copies
before batch work. Run the examples below from that directory:
...
Batch-strip alpha from the working PNG copies:
