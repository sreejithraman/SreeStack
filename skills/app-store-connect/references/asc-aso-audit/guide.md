...
- If metadata came from `asc migrate export` or `asc localizations download`, normalize it into the canonical `./metadata` layout before running this guide.
...
1. Read [references/aso_rules.md](references/aso_rules.md) to understand the rules each check enforces.
...
- Run this guide after `asc metadata pull` to ensure canonical metadata files are current.
