...
## Clipped active labels

Use a decorative copy of the tab row when the active text color and background must change together. Clip the active copy to the selected tab’s bounds; update those bounds when labels, layout, or fonts change. Use the project’s `--ease-in-out`, or start with `cubic-bezier(0.77, 0, 0.175, 1)`.
