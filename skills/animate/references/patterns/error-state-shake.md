...
The reduced-motion rule stops the input animation and removes its transform. Keep the error message visible while the error applies, and bypass timed motion without hiding useful feedback. Follow the [implementation checks](../implementation.md) for JavaScript cancellation and preference changes.
