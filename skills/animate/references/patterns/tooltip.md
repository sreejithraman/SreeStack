...
## Instant movement between nearby tooltips

For a component library with grouped tooltips, use its delay and instant-entry state instead of measuring a shared bubble yourself. Delay the first reveal to avoid stray activation; once one is open, skip both the delay and entry animation on nearby triggers. Use the project’s `--ease-out` token; if absent, start with `cubic-bezier(0.23, 1, 0.32, 1)`.
