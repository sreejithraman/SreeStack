...
## Measured-height alternative

When the existing component supplies content height, animate that value instead of adding the grid wrapper above. Measure content or use its supplied CSS variable, and recheck when content changes. Choose one height strategy. Use the project’s `--ease-out` token; if absent, start with `cubic-bezier(0.23, 1, 0.32, 1)`.
