...
SCRIPTS = ROOT / "skills" / "preview" / "scripts"
if str(SCRIPTS) not in sys.path:
    sys.path.insert(0, str(SCRIPTS))

from previewctl_lib.adapters.ios import (  # noqa: E402
    CommandFailure,
    CommandResult,
    DiscoveryError,
    IOSConfiguration,
...
class FakeRunner:
    ...
    def run(self, argv, *, cwd=None):
        ...
        if command[0:3] == ("xcrun", "simctl", "create"):
            self.devices[TEMPLATE] = self.device(TEMPLATE, "previewctl template", "Shutdown")
...
class IOSAdapterTests(unittest.TestCase):
    ...
    def request(
        self, *, worktree=None, worktree_id="worktree-one", preview_id="pvw_ios_demo", **overrides
    ):
        values = {
    ...
    def test_distinct_profiles_in_one_worktree_get_distinct_owned_clones(self) -> None:
        first = self.adapter().start(self.request(preview_id="pvw_ios_profile_one"))
        second = self.adapter().start(self.request(preview_id="pvw_ios_profile_two"))
        self.assertNotEqual(first.simulator_udid, second.simulator_udid)
        ownership = {record.udid: record for record in self.store.list_ios_simulators()}
        self.assertEqual("pvw_ios_profile_one", ownership[first.simulator_udid].preview_id)
        self.assertEqual("pvw_ios_profile_two", ownership[second.simulator_udid].preview_id)
