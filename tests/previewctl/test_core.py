...
SCRIPTS = Path(__file__).resolve().parents[2] / "skills" / "preview" / "scripts"
sys.path.insert(0, str(SCRIPTS))

from previewctl_lib.core import (  # noqa: E402
    cleanup,
    cleanup_candidates,
    get_record,
    new_record,
...
from previewctl_lib.adapters.base import EvidenceAdapter, RegisteredSurfaceAdapter  # noqa: E402
from previewctl_lib.errors import AdapterError, ConfigurationError, RegistryError  # noqa: E402
from previewctl_lib.paths import state_root  # noqa: E402
from previewctl_lib.project import Manifest, Project, detect_project, load_manifest, preview_id  # noqa: E402
from previewctl_lib.registry import Registry  # noqa: E402
...
class RepositoryCase(unittest.TestCase):
    def setUp(self) -> None:
        ...
        subprocess.run(["git", "-C", str(self.repo), "config", "user.email", "preview@example.invalid"], check=True)
        subprocess.run(["git", "-C", str(self.repo), "config", "user.name", "Preview Tests"], check=True)
...
class ProjectTests(RepositoryCase):
    def test_worktrees_share_project_but_have_distinct_identity_and_preview_ids(self) -> None:
        ...
        self.assertNotEqual(preview_id(first, "web-local", "default"), preview_id(second, "web-local", "default"))
    ...
    def test_manifest_loads_provider_neutral_fields(self) -> None:
        (self.repo / ".preview.toml").write_text(
            """version = 1
kind = "web"
...
""",
        ...
        self.assertEqual(manifest.command[-1], "{port}")
        self.assertEqual(manifest.working_directory, (self.repo / "app").resolve())
        self.assertEqual(manifest.evidence, ("screenshot",))
        self.assertEqual(manifest.ios["scheme"], "Example")

    def test_manifest_rejects_shell_string_and_escape(self) -> None:
        project = detect_project(self.repo)
        for text in ('command = "npm start"\n', 'working_directory = "../outside"\n'):
            with self.subTest(text=text):
                (self.repo / ".preview.toml").write_text(text, encoding="utf-8")
                with self.assertRaises(ConfigurationError):
                    load_manifest(project)

    def test_manifest_rejects_malformed_toml(self) -> None:
        (self.repo / ".preview.toml").write_text("kind = [\n", encoding="utf-8")
        with self.assertRaises(ConfigurationError):
            load_manifest(detect_project(self.repo))

    def test_manifest_auto_detection_and_unknown_key_rejection(self) -> None:
        (self.repo / "package.json").write_text('{"scripts":{"dev":"vite"}}', encoding="utf-8")
        (self.repo / ".preview.toml").write_text('kind = "auto"\n', encoding="utf-8")
        self.assertEqual(load_manifest(detect_project(self.repo)).kind, "web")
        (self.repo / ".preview.toml").write_text('knd = "web"\n', encoding="utf-8")
        with self.assertRaises(ConfigurationError):
            load_manifest(detect_project(self.repo))
...
class RegistryTests(RepositoryCase):
    def test_atomic_registry_preserves_concurrent_updates(self) -> None:
        ...
        def write(index: int) -> None:
            record = new_record(project, manifest, f"adapter-{index}", NOW)
            barrier.wait()
    ...
    def test_state_root_precedence(self) -> None:
        self.assertEqual(state_root({"PREVIEWCTL_STATE_DIR": str(self.root / "custom")}), (self.root / "custom").resolve())
        self.assertEqual(state_root({"HOME": "/Users/example"}, "darwin"), Path("/Users/example/Library/Application Support/previewctl"))
        self.assertEqual(state_root({"HOME": "/home/example", "XDG_STATE_HOME": "/state"}, "linux"), Path("/state/previewctl"))

    def test_created_state_directory_is_private(self) -> None:
        project = detect_project(self.repo)
        self.registry.put(new_record(project, load_manifest(project), "evidence-only", NOW))
    ...
    def test_registry_rejects_missing_nested_fields_and_bad_timestamps(self) -> None:
        ...
        for mutate in (
            ...
            with self.subTest(mutate=mutate):
                record = new_record(project, load_manifest(project), "evidence-only", NOW)
                mutate(record)
                with self.assertRaises(RegistryError):
                    self.registry.put(record)
...
class GenericAdapterTests(unittest.TestCase):
    ...
    def test_registered_url_is_verified_without_provider_deployment_logic(self) -> None:
        ...
        with mock.patch("previewctl_lib.adapters.base.urllib.request.urlopen", return_value=response):
...
class LifecycleTests(RepositoryCase):
    def _start(self) -> dict:
        with mock.patch.dict(os.environ, {"PREVIEWCTL_NOW": "2026-01-02T03:04:05Z"}):
    ...
    def test_failed_start_removes_only_its_reservation(self) -> None:
        ...
        with mock.patch("previewctl_lib.core.load_adapter", return_value=FailingAdapter()):
    ...
    def test_stale_starting_reservation_can_recover(self) -> None:
        project = detect_project(self.repo)
        manifest = load_manifest(project)
        stale = new_record(project, manifest, "evidence-only", datetime(2026, 1, 1, tzinfo=timezone.utc))
    ...
    def test_active_start_reconciles_and_restarts_failed_adapter(self) -> None:
        ...
        adapter = FakeAdapter()
        with mock.patch("previewctl_lib.core.load_adapter", return_value=adapter):
            with mock.patch.dict(os.environ, {"PREVIEWCTL_NOW": "2026-01-02T03:04:05Z"}):
    ...
    def test_expired_active_start_stops_and_replaces_exact_record(self) -> None:
        with mock.patch.dict(os.environ, {"PREVIEWCTL_NOW": "2026-01-01T00:00:00Z"}):
            expired = start(self.registry, self.state, self.repo, "evidence-only")
        with mock.patch.dict(os.environ, {"PREVIEWCTL_NOW": "2026-01-03T00:00:00Z"}):
    ...
    def test_stale_starting_reservation_invokes_exact_adapter_recovery(self) -> None:
        project = detect_project(self.repo)
        manifest = load_manifest(project)
        stale = new_record(project, manifest, "recoverable", datetime(2026, 1, 1, tzinfo=timezone.utc))
        self.registry.put(stale)
        ...
        with mock.patch("previewctl_lib.core.load_adapter", return_value=adapter), mock.patch.dict(
            os.environ, {"PREVIEWCTL_NOW": "2026-01-02T03:04:05Z"}
        ):
    ...
    def test_verify_preserves_a_concurrent_pin(self) -> None:
        ...
        with mock.patch("previewctl_lib.core._load_record_adapter", return_value=PinningAdapter()):
    ...
    def test_verify_refuses_to_overwrite_a_concurrent_stop_claim(self) -> None:
        ...
        with mock.patch("previewctl_lib.core._load_record_adapter", return_value=RacingAdapter()):
    ...
    def test_manual_cleanup_policy_has_no_automatic_expiration(self) -> None:
        (self.repo / ".preview.toml").write_text('cleanup_policy = "manual"\n', encoding="utf-8")
        with mock.patch.dict(os.environ, {"PREVIEWCTL_NOW": "2026-01-02T03:04:05Z"}):
            record = start(self.registry, self.state, self.repo, "evidence-only")
        self.assertIsNone(record["timestamps"]["expires_at"])
        self.assertEqual([], cleanup_candidates([record], datetime(2027, 1, 1, tzinfo=timezone.utc)))
    ...
    def test_cleanup_dry_run_is_byte_for_byte_immutable(self) -> None:
        ...
        with mock.patch.dict(os.environ, {"PREVIEWCTL_NOW": "2026-01-04T00:00:00Z"}):
    ...
    def test_register_is_idempotent_and_preserves_pin(self) -> None:
        ...
        with mock.patch.dict(os.environ, {"PREVIEWCTL_NOW": "2026-01-03T03:04:05Z"}):
    ...
    def test_register_normalizes_relative_artifact_and_evidence_paths(self) -> None:
        ...
        self.assertEqual(record["log_paths"], [str((self.repo / "preview.log").resolve())])
