...
LIB = ROOT / "skills" / "showroom" / "scripts"
if str(LIB) not in sys.path:
    sys.path.insert(0, str(LIB))

from showroom_lib.adapters.base import AdapterContext
from showroom_lib.adapters.web_local import Adapter, UrllibProbe
from showroom_lib.errors import AdapterError
from showroom_lib.project import ProjectConfig, Project
...
def make_context(root: Path, approvals: dict[str, bool] | None = None) -> AdapterContext:
    ...
    return AdapterContext(
        ...
        showroom_dir=root / "state" / "showrooms" / "srm_test",
...
class WebLocalAdapterTests(unittest.TestCase):
    def test_http_probe_polls_until_a_development_server_is_ready(self) -> None:
        ...
        with mock.patch(
            "showroom_lib.adapters.web_local.urllib.request.urlopen",
            side_effect=[urllib.error.URLError("not ready"), response],
        ) as urlopen, mock.patch(
            "showroom_lib.adapters.web_local.time.monotonic",
            side_effect=[0.0, 0.1, 0.2, 0.3],
        ), mock.patch("showroom_lib.adapters.web_local.time.sleep"):
    ...
    def test_start_creates_exact_private_route_and_verified_patch(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            patch = adapter.start({"id": "srm_test", "resources": {}}, make_context(Path(directory)))
            ...
            self.assertIn((
                ...
                "bootstrap",
                "gui/501",
                str(Path(directory) / "state" / "showrooms" / "srm_test" / "origin.plist"),
            ), runner.calls)
            self.assertEqual(
                [
        ...
        for approvals in (
            ...
            with self.subTest(approvals=approvals), tempfile.TemporaryDirectory() as directory:
                ...
                with self.assertRaises(AdapterError):
                    adapter.start({"id": "srm_test", "resources": {}}, context)
                self.assertEqual([], runner.calls)
                self.assertFalse(context.showroom_dir.exists())
    ...
    def test_bootstrap_failure_leaves_no_owned_process_route_or_metadata(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertFalse((context.showroom_dir / "origin.plist").exists())
            self.assertFalse((context.showroom_dir / "ownership.json").exists())
    ...
    def test_recover_removes_exact_resources_from_an_interrupted_start(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            record = {"id": "srm_test", "status": "starting", "resources": {}}
    ...
    def test_origin_health_failure_boots_out_exact_launchagent(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertTrue(bootouts[0][-1].startswith("gui/501/com.sreestack.showroom."))
    ...
    def test_rollback_never_removes_a_route_that_changed_ownership(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with self.assertRaisesRegex(AdapterError, "tailnet health"):
                adapter.start({"id": "srm_test", "resources": {}}, make_context(Path(directory)))
    ...
    def test_failed_rollback_preserves_exact_recovery_metadata(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertTrue(runner.launchd_loaded)
            self.assertIsNone(runner.route)
            self.assertTrue((context.showroom_dir / "origin.plist").is_file())
            self.assertTrue((context.showroom_dir / "ownership.json").is_file())
